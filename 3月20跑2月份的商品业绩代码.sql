--3月20跑2月份的商品维度毛利额(排出退款单)
-- 自营和POP分开跑，一次跑可能跑不出来。
--cc_ods_op_order_batches where条件里ds不能省略不写。得是'${begin_date}'的前一天。不然的话也可能跑不出来。
select
...
(
    select
        ...
        m1.shop_id as shop_id,
        m1.shop_title as shop_title,
        ...
        ( 
            ...
            (
                ...
                (
                    ...
                    where
                        from_unixtime(create_time,'yyyyMMdd')>=20190201
                ) n2
    ...
    union all
    select
        m2.product_id as product_id,
        m2.product_cname1 as product_cname1,
        m2.product_cname2 as product_cname2,
        m2.shop_id as shop_id,
        m2.shop_title as shop_title,
...
group by h1.product_id,h1.product_cname1,h1.product_cname2,h1.shop_id,h1.shop_title
