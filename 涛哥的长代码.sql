...
( 
    SELECT
        ...
        max(shop_type)                 as shop_type,--店铺类型
        sum(cck_commission_30)         as cck_commission_30,--30日佣金
        sum(pay_fee_30)                as pay_fee_30,--30日支付金额
        sum(cck_commission)            as cck_commission,--指定日期佣金
        sum(pay_fee)                   as pay_fee,--指定日期支付金额
        sum(add_fee)                   as add_fee,
        sum(ipv_30)                    as ipv_30,--30日ipv
        sum(ipv)                       as ipv,--指定日期ipv
        sum(bad_cnt)                   as bad_cnt,--指定日期差评数
        sum(eva_rate_cnt)              as eva_rate_cnt,--指定日期评价数
        sum(bad_cnt_30)                as bad_cnt_30,--30日差评数
        sum(eva_rate_cnt_30)           as eva_rate_cnt_30,--30日评价数
        sum(fx_cnt)                       as fx_cnt,--指定日期分享数
        sum(if(pay_cnt>0,1,0))            as saled_prd_cnt--指定日期动销数
...
(
  SELECT
    ...
    count(distinct ad_id)  as online_prd_cnt,--在线商品数
    sum(if(start_time>='${bizdate_ts}' and start_time<'${gmtdate_ts}',1,0)) as new_prd_cnt--上新商品数
